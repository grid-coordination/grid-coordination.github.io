...
History shows that open standards in energy do not emerge from market forces alone. HVAC manufacturers have moved *away* from open control interfaces toward proprietary lock-in. The water heater industry shows what regulation can achieve, and also where a physical requirement runs out. Washington, Oregon and Colorado prohibit the sale of a covered electric water heater that lacks a CTA-2045/EcoPort port, and New York reaches new construction at permit through its building energy code. The compliant models exist because of those rules and would not exist otherwise: nobody was going to build them voluntarily. But a port costs money and only a few states require it, so it ships as a demand-response model variant rather than across the line. Rheem's September 2025 compliance bulletin lists its demand-response models by state, and the least expensive of them carries the port and no Wi-Fi at all. The lesson is not that regulation failed. It is that the instrument matters: a requirement met in firmware travels with every unit a manufacturer builds, while a requirement met with a physical port splits the product line and stops at the state border.
